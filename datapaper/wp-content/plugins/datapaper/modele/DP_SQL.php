...
class DP_SQL {
    ...
    public static function DP_add_user($username, $user_email , $uri) {
        $username = trim($username);
        $username = str_ireplace(" ", "", $username);
        $id= wp_create_user($username, wp_generate_password(), $user_email);

      //  DP_insert_uri($uri , $id);
             //   $id= wp_create_user($user_email, wp_generate_password(), $user_email);
 
             //   DP_insert_mail_encryp($user_email,$id);
            //  DP_insert_mail_encryp_name($user_email,$username,$id);
// Set the nickname
       /*         
  wp_update_user(
    array(
...
*/
    ...
    public static function DP_insert_uri($uri,$id){
        global $wpdb;
        $sql='update '.Install::getPrefix4().' set uri=\''.$uri.'\' WHERE id='.$id. ' ;';
    ...
    public static function DP_insert_mail_encryp($mailEncryp,$id){
     ...
     public static function DP_insert_mail_encryp_name($mailEncryp,$username,$id, $uri){
        global $wpdb;
        $sql='update '.Install::getPrefix4().' set mail_encryp=\''.$mailEncryp.'\',user_nicename=\''.$username.'\' , display_name=\''.$username.'\' ,uri=\''.$uri.'\' WHERE id='.$id. ' ;';
        $result = $wpdb->query($sql );

        
        //, user_email=\''.$mailEncryp.'\'
    }
