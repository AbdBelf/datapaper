...
/**
* Liste des utilisateurs retournés a partir du EndPoint Sparql et qui n'existe pas dans 
note bdd wordpress
*/
...
function show_list_unique(vector) {
    ...
    for (result in vector) {
       ...
       out+='<option value="'+vector[result].mailEncryp.value+'" uri="'+vector[result].uri.value+'">'+vector[result].name.value+ '</option>';
...
/**
*
*  Fonction d'ajout des utilisateurs après vérification du mail.
*/
function add_all_users(vector){
    
var data;
console.log('vector');
console.log(vector);
  //  console.log('user_info '+user_info.name.value+ ' '+ user_info.mailEncryp.value);
   var i=0;
    for (result in vector) {
      ...
      if(i <2)
      {
       ...
       console.log('Name :'+vector[result].name.value + ' mailEncryp : '+vector[result].mailEncryp.value);

       // console.log('result'+result);
   
   var user_info = vector[result];


    var mail=user_info.mailEncryp.value;
    
    console.log('mail encryp:'+mail);
    ...
    var uri = user_info.uri.value;


    console.log('name :'+name);
       data2 = {
        action: 'add_user',
        value: {user_mail:mail,id_source:id_source,name:name , uri:uri}
    };
    console.log('data2');
    console.log(data2);
    
    var user=DP_ajax_server(data2);
...
function add_user(){
    ...
    console.log('name :'+name);
     ...
     var uri=jQuery('#dp-user-list :selected ');
     console.log('uri '.uri);
  //  var uri = user_info.uri.value;  , uri:uri
      var data = {
        action: 'add_user',
