...
<input type="hidden" name="user_id" id="user_id" value="<?php echo esc_attr($user_id); ?>" />
 ...
 <div id="datapaper_command" class="buttoncontainer" >
         
              <button type="button" id="datapaper_add_more_info" class="btn btn-success" onclick="DP_addMoreInfo(null,true)">Add More Info</button>
...
//change style button submit 

jQuery( "#submit" ).css({ width: '130px' , height : '30px'});


//On submit , reload Nosql Data
...
function DP_searchValeu(idobj) {



    id = idobj;// jQuery(obj).attr('email');
    ...
    if (value !== null) {
        ...
        for (var j = 0; j < value.public.length; j++) {
            ...
            for (key in value.public[j]) {
            	...
            	//pas la peine de generer tout formulaire hors foaf car il est deja dans le profile wp
            	if (  key.indexOf("foaf") == -1 )
            		continue;
...
/*
    out += '<tr>';
    out+= '<td><select id="'+i+'foaf" onchange="Add_TextArea(1,'+i+')" ></select> <button id="changepageDce'+i+'" >+</button></td>';
...
//onclick="clickDCEButton('+i+')"
   ...
   */
     ...
     out2 = '<select id="'+i+'foaf" onchange="Add_TextArea(1,'+i+')"  ></select> <button class="btn btn-success button-form-control" id="changepageDce'+i+'" >+</button>';
...
/*
var user_name = jQuery("#user_login").value;
var first_name = jQuery("#first_name").value;
var last_name = jQuery("#last_name").value;
var nickname = jQuery("#nickname").value;
var url = jQuery("#url").value;
var yim = jQuery("#yim").value;
var description = jQuery("#description").value;
var img = jQuery("#avatar avatar-100").value;

*/


//Pour récupérer les données du formulaire
var obj = {user_name: jQuery("#user_login").val() , 
			...
			img : jQuery("#avatar avatar-100").val()

			};

jQuery(obj).serializeObject()
var data = {
        action: 'send_info',
...
//console.log(data.value.public[1].foaf_givenName);
var object = jQuery.extend( obj  ,  data.value.public[1]);

data.value.public[1] = obj;
...
function Add_TextArea(i,idFoaf) {

    if (notGenerated){
         var selectBox = document.getElementById(idFoaf+"foaf");
  ...
  //  out += '<td><input type="text" name="public[' + i + ']['+ value +']" value=""></td>';
    out2 = '<input class=form-control id="input'+i+'" type="text" name="public[' + i + ']['+ value +']" value="">';
...
function getLovListProperties(type,page,idFoaf){
        var jsonData = "";
    ...
    dropdown = dropdown + "<option value='"+property+"'>"+item.localName+"</option>"
                  ...
                  jQuery("#"+idFoaf+type).append(dropdown); 
            });
        return jsonData;
      };
...
idFoaf = event.data.i;
            pageDce = pageDce +1;
        // alert(IdDce)
            var results = getLovListProperties("foaf",pageDce,idFoaf);
            console.log(results);
